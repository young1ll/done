...
#   pw-daemon.sh run                     # foreground (what the service runs)
...
CFG="$PW_HOME/config.json"; PIDF="$PW_HOME/daemon.pid"; SPIDF="$PW_HOME/sync.pid"; LOG="$PW_HOME/daemon.log"; SLOG="$PW_HOME/sync.log"
...
start_sync() { nohup node "$HERE/pw-sync.mjs" --port "$PW_DEBUG_PORT" --state "$PW_STATE" >> "$SLOG" 2>&1 < /dev/null & echo $! > "$SPIDF"; }
...
case "${1:-status}" in
  start)
    ...
    if alive "$PIDF"; then echo "already running (pid $(cat "$PIDF")) on http://localhost:$PW_MCP_PORT/mcp"; alive "$SPIDF" || start_sync; exit 0; fi
    ...
    echo $! > "$PIDF"; start_sync
    for i in $(seq 1 90); do listening "$PW_MCP_PORT" && break; sleep 0.5; done
    if listening "$PW_MCP_PORT"; then echo "started pid $(cat "$PIDF") (sync $(cat "$SPIDF")) — MCP http://localhost:$PW_MCP_PORT/mcp, home $PW_HOME"
    else echo "did not come up in 45s — see $LOG" >&2; tail -5 "$LOG" >&2; exit 1; fi;;
  run)   # foreground, for the service manager
    [ -f "$PW_HOME/disabled" ] && { echo "disabled"; exec sleep 3600; }
    prepare; cd "$PW_HOME/output" || exit 1   # files saved by name land next to the timestamped ones; echo $$ > "$PIDF"; start_sync
    trap 'stop_pid "$SPIDF"; rm -f "$PIDF"' EXIT
    exec npx "$PW_MCP_PKG" "${ARGS[@]}" >> "$LOG" 2>&1;;
  stop) stop_pid "$SPIDF"; if alive "$PIDF"; then p=$(cat "$PIDF"); stop_pid "$PIDF"; echo "stopped $p"; else rm -f "$PIDF"; echo "not running"; fi
        [ -f "$(unit_path)" ] && echo "note: the login service is installed and will restart it — use 'disable' or 'uninstall' to keep it down";;
  ...
  status)
    ...
    [ -f "$(unit_path)" ] && echo "  login service: installed ($(unit_path))" || echo "  login service: not installed (pw-daemon.sh install)";;
  export)
  ...
  disable) touch "$PW_HOME/disabled"; "$SELF" stop >/dev/null; case "$(uname -s)" in Darwin) [ -f "$(unit_path)" ] && launchctl unload "$(unit_path)" 2>/dev/null;; *) systemctl --user stop "$LABEL" 2>/dev/null;; esac; echo "disabled — hook and service will not start it until: $SELF enable";;
  enable) rm -f "$PW_HOME/disabled"; case "$(uname -s)" in Darwin) [ -f "$(unit_path)" ] && launchctl load "$(unit_path)" 2>/dev/null;; *) systemctl --user start "$LABEL" 2>/dev/null;; esac; echo "enabled";;
  install)
    NODE_BIN=$(dirname "$(command -v node)"); NPX=$(command -v npx) || { echo "npx not found" >&2; exit 1; }
...
PLIST
        ...
        "$SELF" stop >/dev/null 2>&1; launchctl unload "$u" 2>/dev/null; launchctl load "$u" && echo "installed $u — starts at login, restarts if it dies";;
      *)
        cat > "$u" <<UNIT
[Unit]
...
UNIT
        [ "${2:-}" = "--print" ] && { cat "$u"; rm -f "$u"; exit 0; }
        "$SELF" stop >/dev/null 2>&1; systemctl --user daemon-reload && systemctl --user enable --now "$LABEL" && echo "installed $u — starts at login, restarts if it dies";;
    esac
    for i in $(seq 1 90); do listening "$PW_MCP_PORT" && break; sleep 0.5; done; "$SELF" status | head -1;;
  uninstall)
    u=$(unit_path); case "$(uname -s)" in Darwin) launchctl unload "$u" 2>/dev/null;; *) systemctl --user disable --now "$LABEL" 2>/dev/null;; esac
    rm -f "$u"; "$SELF" stop >/dev/null; echo "uninstalled $u";;
